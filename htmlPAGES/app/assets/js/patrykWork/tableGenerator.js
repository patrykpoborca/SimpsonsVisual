...
{
//init
if(isNaN(rows))rows = Number(rows); if(isNaN(columns))columns = Number(columns); //incase string input.
var divString = "#"+divID;
...
if(!data['type']) data['type'] = "generatorTypeFlag = 'NaN' ";
}
...
if(cellDimX < 5) cellDimX = 5;
if(cellDimY < 5) cellDimY = 5;
	...
	for(C = 0; C < data[R].length; C++)
	{
		...
		else
		{
			if(!display)
				...
				{
				if(data[R][C]["color"]) color = (data[R][C]["data"]) ? data[R][C]["color"] : colors["no"];
				else
